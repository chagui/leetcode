...
class Solution {
  public:
    ...
    int largestOverlapBruteForce(vector<vector<int>> &img1,
        ...
        const auto count_permutations_overlap =
            [&permutations_overlap](
                vector<vector<int>> &img1, vector<vector<int>> &img2,
        ...
        for (int vertical_offset = 0; vertical_offset < size;
             ++vertical_offset) {
    ...
    int largestOverlapBitSet(vector<vector<int>> &img1,
        ...
        vector<int> permutations_overlap(4);
        const auto count_permutations_overlap =
            [size, &permutations_overlap, &image1,
             &image2](int horizontal_offset, int vertical_offset) {
                permutations_overlap = {0, 0, 0, 0};
                for (int i = 0, io = vertical_offset;
                     i < size - vertical_offset; ++i, ++io) {
                    // move left-up
