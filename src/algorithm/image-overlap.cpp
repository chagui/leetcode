   ...
   public:
    ...
    int largestOverlapBruteForce(vector<vector<int>> &img1, vector<vector<int>> &img2) {
        ...
        const auto count_permutations_overlap =
            [&permutations_overlap](
                vector<vector<int>> &img1, vector<vector<int>> &img2, int horizontal_offset, int vertical_offset) {
        ...
        for (int vertical_offset = 0; vertical_offset < size; ++vertical_offset) {
            for (int horizontal_offset = 0; horizontal_offset < size; ++horizontal_offset) {
                ...
                const int current_largest_overlap =
                    count_permutations_overlap(img1, img2, vertical_offset, horizontal_offset);
    ...
    int largestOverlapBitSet(vector<vector<int>> &img1, vector<vector<int>> &img2) {
        ...
        vector<int> permutations_overlap(4);
        const auto count_permutations_overlap = [size, &permutations_overlap, &image1, &image2](int horizontal_offset,
                                                                                                int vertical_offset) {
            permutations_overlap = {0, 0, 0, 0};
            for (int i = 0, io = vertical_offset; i < size - vertical_offset; ++i, ++io) {
                // move left-up
                permutations_overlap[0] += ((image1[io] << horizontal_offset) & image2[i]).count();
                // move right-up
                permutations_overlap[1] += ((image1[io] >> horizontal_offset) & image2[i]).count();
                // move right-down
                permutations_overlap[2] += ((image1[i] >> horizontal_offset) & image2[io]).count();
                // move left-down
                permutations_overlap[3] += ((image1[i] << horizontal_offset) & image2[io]).count();
            }
            return *max_element(permutations_overlap.cbegin(), permutations_overlap.cend());
        };
        ...
        for (int vertical_offset = 0; vertical_offset < size; ++vertical_offset) {
            for (int horizontal_offset = 0; horizontal_offset < size; ++horizontal_offset) {
                // if the potential overlap is not big enough break (it will
                // only get smaller)
                const int potential_overlap = (size - vertical_offset) * (size - horizontal_offset);
