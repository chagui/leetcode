...
public:
  ...
  string removeKdigitsRec(string num, int k) {
    if (k >= num.length() || num.empty())
      return "";
    if (k == 0 || num.length() == 1)
      return num;
    if (num.length() == 2)
      return (num.front() < num.back()) ? string(1, num.front())
                                        : string(1, num.back());
    // best case scenario
    if (num[1] == '0') {
      auto first_non_zero = find_if_not(++num.begin(), num.end(), isZero);
      if (first_non_zero == num.end())
        return "";
      return removeKdigitsRec(num.substr(distance(num.begin(), first_non_zero)),
                              k - 1);
    }
    ...
    if (length_diff == 0)
      return (without_front.front() < num.front()) ? without_front
                                                   : num.front() + with_front;
    return (with_front.length() > without_front.length()) ? with_front
                                                          : without_front;
  }
  ...
  string removeKdigits(string num, int k) {
    ...
    if (it != num.end())
      copy(it, num.end(), back_inserter(result));
